...
const getSnippetsInfo = async (args: {recording: Recording, sorting: Sorting, unitId: number | number[]}): Promise<InfoType> => {
    const { recording, sorting, unitId } = args
...
const getSnippets = async (args: {recording: Recording, sorting: Sorting, unitId: number | number[], timeRange: {min: number, max: number}}): Promise<Snippet[]> => {
        ...
        async (query: QueryType) => {
            switch(query.type) {
                case 'info':
                    ...
                    return await getSnippetsInfo({recording: query.recording, sorting: query.sorting, unitId: uid1})
                case 'snippets':
                    const uid2 = applyMergesToUnit(query.unitId, curation, selection.applyMerges)
                    return await getSnippets({recording: query.recording, sorting: query.sorting, unitId: uid2, timeRange: query.timeRange})
            }
        }
    ), [curation, selection.applyMerges])
