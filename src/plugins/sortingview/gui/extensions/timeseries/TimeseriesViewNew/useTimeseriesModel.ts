...
const getTimeseriesDataSegment = async (args: {recordingObject: any, ds_factor: number, segment_num: number, segment_size: number}): Promise<number[][] | undefined> => {
  const { recordingObject, ds_factor, segment_num, segment_size } = args
...
const useTimeseriesData = (recordingObject: any, recordingInfo: RecordingInfo): TimeseriesData | null => {
  const fetch = useMemo(() => (async (query: TimeseriesDataQuery) => {
    switch(query.type) {
      case 'dataSegment': {
        return await getTimeseriesDataSegment({recordingObject, ds_factor: query.ds_factor, segment_num: query.segment_num, segment_size: query.segment_size})
      }
    }
  }), [recordingObject])
