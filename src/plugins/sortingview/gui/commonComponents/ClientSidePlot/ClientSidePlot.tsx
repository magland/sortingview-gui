...
const ClientSidePlot: FunctionComponent<{
    ...
    useEffect(() => {
        ;(async () => {
            if ((calculationStatus === 'waitingForVisible') && (visible)) {
                ...
                let plot_data;
                try {
                    plot_data = await runPureCalculationTaskAsync(dataFunctionName, dataFunctionArgs, {})
                }
                catch (err: any) {
                    console.error(err);
    ...
    }, [dataFunctionName, calculationStatus, calculationPool, dataFunctionArgs, visible])
